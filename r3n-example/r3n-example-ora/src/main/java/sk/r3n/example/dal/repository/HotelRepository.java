...
public class HotelRepository {
    ...
    public HotelDto insertHotel(final HotelDto hotelDto) {
        ...
        final int idIndex = sql.getParams().size() + 1;

        final Long id = jdbcTemplate.execute(
                connection -> {
                    final CallableStatement cs = connection.prepareCall(sql.toSql());
                    sqlBuilder.setParams(cs, sql.getParams().toArray(new SqlParam[0]));
                    cs.registerOutParameter(idIndex, Types.BIGINT);
                    return cs;
                },
                (CallableStatementCallback<Long>) cs -> {
                    cs.execute();
                    return cs.getLong(idIndex);
                }
        );
        assert id != null;
        row[0] = id;
