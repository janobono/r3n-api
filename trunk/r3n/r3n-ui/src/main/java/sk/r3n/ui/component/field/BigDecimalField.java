...
public final class BigDecimalField extends R3NField<BigDecimal> {
	...
	private static final long serialVersionUID = 7894043908556320510L;
	...
	public static BigDecimal parseBigDecimal(String string, char separator) {
		...
		try {
			if (string.indexOf(separator) != -1) {
				numerics = Long.toString(Long.parseLong(string.substring(0,
	...
	public static String toString(BigDecimal value, short scale, char separator) {
		...
		try {
			if (result.indexOf('.') != -1) {
				numerics = Long.toString(Long.parseLong(result.substring(0,
						result.indexOf('.'))));
	...
	public BigDecimalField(boolean canBeNull, short scale) {
		super(canBeNull);
		MINUS_SIGN = DecimalFormatSymbols.getInstance(Locale.getDefault())
				.getMinusSign();
