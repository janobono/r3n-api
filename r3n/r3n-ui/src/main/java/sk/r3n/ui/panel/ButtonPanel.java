...
public class ButtonPanel extends JPanel {
    ...
    public ButtonPanel(int rows, int columns) {
        ...
        actual_column = 0;
        if (rows <= columns) {
            this.backward = UIServiceManager.getDefaultUIService().getIdActionService().getKeyStroke(UIService.class.getCanonicalName(),
                    UIService.ACTION_LEFT);
            this.forward = UIServiceManager.getDefaultUIService().getIdActionService().getKeyStroke(UIService.class.getCanonicalName(),
    ...
    public void addButton(R3NButton button) {
        if (actual_row < rows) {
            ...
            if (backward != null && forward != null) {
                ...
                keys.addAll(button.getFocusTraversalKeys(KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS));
                keys.add(backward);
                button.setFocusTraversalKeys(
                        KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS, keys);
                keys.clear();
                keys.addAll(button.getFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS));
                keys.add(forward);
                button.setFocusTraversalKeys(
