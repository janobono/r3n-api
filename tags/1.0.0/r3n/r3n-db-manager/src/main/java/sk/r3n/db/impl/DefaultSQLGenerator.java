...
import sk.r3n.db.Condition;
import sk.r3n.db.Operator;
import sk.r3n.db.SQLColumn;
import sk.r3n.db.SQLCondition;
	...
	public String toSQL(boolean tablePrefix, SQLCondition[][] conditions) {
		...
		for (int i = 0; i < conditions.length; i++) {
			...
			if (cond.length() > 0) {
				result.append(cond);
				if (i < conditions.length - 1)
					result.append(toSQLOperator(conditions[i][conditions[i].length - 1]
							.getOperator()));
			}
		}
		return result.toString();
