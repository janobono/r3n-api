...
public class R3NKeyStrokeDialog extends JDialog implements IdActionExecutor,
	...
	private static final long serialVersionUID = 8729775415623355940L;
	...
	private void init() {
		...
		setTitle(ResourceBundle.getBundle(this.getClass().getCanonicalName())
				.getString("TITLE"));
		...
		if (keyStrokes != null)
			for (KeyStroke exStroke : keyStrokes) {
				if (keyStroke.getKeyCode() == exStroke.getKeyCode()
						...
						&& UIServiceManager.getDefaultUIService()
								.showYesNoDialog(
										null,
										ResourceBundle.getBundle(
												R3NKeyStrokeDialog.class
														.getCanonicalName())
												.getString("EXISTS")
												+ "\n["
												+ keyStroke.toString()
