...
public final class R3NDatePicker extends JPanel implements IdActionExecutor,
		MouseListener {

	private static final long serialVersionUID = 1483291178588344156L;

	private class PickerDayField extends JLabel {

		private static final long serialVersionUID = -7191117354914046246L;
	...
	public R3NDatePicker(Date date) {
		...
		previousButton = new R3NButton(UIService.class.getCanonicalName(),
				UIService.ACTION_PREVIOUS);
		previousButton.addActionListener(new IdActionListener(UIService.class
				.getCanonicalName(), UIService.ACTION_PREVIOUS, this));
		navigationPanel.add(previousButton, new GridBagConstraints(0, 0, 1, 1,
		...
		navigationPanel.add(monthAndYear, new GridBagConstraints(1, 0, 1, 1,
				1.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(0, 0, 0, 0), 0, 0));
		nextButton = new R3NButton(UIService.class.getCanonicalName(),
				UIService.ACTION_NEXT);
		nextButton.addActionListener(new IdActionListener(UIService.class
				.getCanonicalName(), UIService.ACTION_NEXT, this));
		navigationPanel.add(nextButton, new GridBagConstraints(2, 0, 1, 1, 0.0,
				0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(0, 0, 0, 0), 0, 0));
		add(navigationPanel, new GridBagConstraints(0, 0, 1, 1, 1.0, 0.0,
		...
		for (String string : shortWeekdays) {
			...
			header.add(pickerDayField, new GridBagConstraints(column, 0, 1, 1,
		...
		add(header, new GridBagConstraints(0, 1, 1, 1, 1.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(
						0, 0, 0, 0), 0, 0));

		form = new JPanel(new GridBagLayout());
		form.setPreferredSize(new Dimension(size.width * 7 + 7,
				size.height * 7 + 7));
		add(form, new GridBagConstraints(0, 2, 1, 1, 1.0, 1.0,
	...
	private void createDays(Calendar calendar) {
		...
		if (dayFields != null) {
			...
			dayFields = new ArrayList<PickerDayField>();
		}
		...
		for (int i = 1; i <= calendar.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
			...
			if (calendar2.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY
				...
				pickerDayField.setForeground(Color.RED);
			form.add(pickerDayField, new GridBagConstraints(column, row, 1, 1,
					0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
		...
		dayFields.get(calendar.get(Calendar.DAY_OF_MONTH) - 1)
				.setSelected(true);
		revalidate();
		repaint();
	}
