...
public class R3NPropertiesImpl implements R3NProperties {
	...
	private static final Logger LOGGER = Logger.getLogger(R3NProperties.class
			.getCanonicalName());
		...
		try {
			...
			LOGGER.log(
					...
					ResourceBundle.getBundle(
							R3NPropertiesImpl.class.getCanonicalName())
							.getString("LOAD"), e);
		} finally {
			try {
				if (in != null)
	...
	public String decrypt(String string) {
		...
		try {
			...
			return new String(encrypter.decrypt(new HexBinaryAdapter()
					.unmarshal(string)));
		} catch (Exception e) {
	...
	public String encrypt(String string) {
		...
		try {
			Encrypter encrypter = new Encrypter();
			return new HexBinaryAdapter().marshal(encrypter.encrypt(string
					.getBytes()));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
